...
import { FaCamera, FaUser, FaHistory, FaStar, FaTicketAlt, FaCalendarAlt, FaSpinner } from 'react-icons/fa';
...
const ProfilePage = () => {
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      setIsUploading(true);
      // Gọi API PUT (dựa trên route bạn đã cung cấp)
                       ...
                       <div key={booking._id} className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 flex flex-col md:flex-row gap-4 hover:border-orange-500/50 transition group">
                         ...
                         <div className="flex-1">
                           ...
                           <div className="text-sm text-gray-400 mt-2 space-y-1">
                              ...
                              <p className="flex items-center gap-2"><FaMapMarkerAlt className="text-red-500"/> {booking.showtimeId?.room?.name}</p>
                           </div>
                         </div>
